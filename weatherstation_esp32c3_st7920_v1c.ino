 ...
 * - Sincronizare NTP extrem de stabilă, folosind ceasul hardware RTC intern 
 *   al ESP32 și funcțiile POSIX (setenv/tzset). Nu mai folosește biblioteca
 *   veche NTPClient care genera erori de salt al datei.
 ...
 * - Cicluri de afișare (3 secunde / ecran):
 *   1. Dată completă (ZZ.LL.AAAA)
 *   2. Ziua din săptămână
 *   3. Temperatura (grade Celsius, cu semn + sau -)
 *   4. Umiditatea relativă (%)
 *   5. Starea vremii (Senin, Noros, Ploaie, etc.)
 *   6. Vânt (Direcție cardinală RO: N, NE, SV, V + viteză în km/h)
 *   7. Presiune atmosferică la nivelul mării (convertită din hPa în mmHg)
 *   8. Indice UV (Afișat DOAR ziua când > 0.1, însoțit de calificativ)
 *   9. Calitatea aerului (European AQI, însoțit de calificativ)
 * 
 * HARDWARE NECESAR:
 * - Placă: ESP32-C3 Mini (sau orice ESP32 compatibil).
 * - Ecran: LCD ST7920 128x64 (conectat prin Software SPI pentru a elibera pinii hardware).
 * 
 ...
 * (Restul librăriilor: WiFi, WebServer, HTTPClient, time sunt preinstalate cu ESP32)
 * ==============================================================================
 */
...
// Coordonate GPS (trebuie modificate cu locația ta exactă)
...
void setup()
{
    ...
    // === SOLUȚIA PENTRU ESP32 CORE 2.0.5 ===
    // Setăm fusul orar al Romanie (EET) și regulile DST (ora de vară EEST)
    setenv("TZ", "EET-2EEST,M3.5.0/3,M10.5.0/4", 1);
    tzset(); 
    
    // Pornim clientul NTP intern
